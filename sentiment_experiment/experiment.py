"""
Experiment to verify the performance difference between using a simple trading strategy and one that takes advantage
of news sentiment as a feature.
"""
import tempfile
...
def run_experiment(train, test, symbol, required_headline_content, commission, initial_cash, obs_columns):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        all_stats, all_trades = multi_day_backtesting(test, initial_cash, obs_columns, "LongOnlyStockTradingEnv-v0", temp_dir/"best_model.zip", verbose=True)
...
if __name__ == "__main__":
    ...
    mlflow.set_experiment("sentiment_impact_trading_with_60_decay")
    runs = 40
    symbol = "MSFT"
    required_headline_content = ["microsoft"]
    news_sentiment_pipeline = generate_news_with_sentiment_pipeline(symbol, "orca2", datetime(2010, 1, 1),
                                                                    datetime(2024, 2, 15), required_headline_content
                                                                    )
    with_sentiment_decay = True
    bar_data_pipeline = generate_candle_data_pipeline(symbol, datetime(2010, 1, 1), datetime(2024, 2, 15),
                                                      with_sentiment=True,
                                                      news_sentiment_pipeline=news_sentiment_pipeline, with_sentiment_decay=with_sentiment_decay)


    data = bar_data_pipeline.transform(None)

    train = data.loc[datetime(2010, 9, 13):datetime(2023, 1, 15)]
    test = data.loc[datetime(2023, 1, 15):datetime(2024, 12, 30)]
    for i in range(runs):
        with mlflow.start_run():
            mlflow.log_param("with_decay", with_sentiment_decay)
            run_experiment(train, test, "MSFT", ["microsoft"], 0.0003, 100_000,
                           CANDLE_DATA_OBSERVABLE_WITH_SENTIMENT_COLUMNS)
        with mlflow.start_run():

            run_experiment(train, test, "MSFT", ["microsoft"], 0.0003, 100_000, CANDLE_DATA_OBSERVABLE_COLUMNS)
